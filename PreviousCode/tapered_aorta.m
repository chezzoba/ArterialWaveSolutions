...
% E = 400e3;
% v = 0.5;
% bet = 0.00226179262436466;
% L = 0.207491876088907;
% R = 0.0131695502637846;
% RW1 = 19783915.9077950;
% RW2 = 103554052.870129;
% Cwk = 1.03245297963988e-08;
% h = (1-v^2)/(E*bet); 
...
load('automeris/tapered_aorta/outlet_flow.csv')
...
%% Error calculations
%Average
yiinpressure = interp1q(Inlet_pressure(1:end,1),Inlet_pressure(1:end,2),xi);
for i = 2:length(t)-2
...
emaxinletpressure = max(abs((p1(2:end-1)-yiinpressure(2:end-1))/max(yiinpressure(2:end-1))));
...
esysinletpressure = (max(p1(2:end-1)-max(yiinpressure(2:end-1))))/max(yiinpressure(2:end-1));
...
ediasinletpressure = (min(p1(2:end-1)-min(yiinpressure(2:end-1))))/max(yiinpressure(2:end-1));
